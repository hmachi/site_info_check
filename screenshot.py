def get_concat_image(img1, img2, direction='v'):
    ...
    if direction == 'v':
        new_img = Image.new(
            'RGB', (max(img1.width, img2.width), (img1.height + img2.height)))
...
def get_full_screenshot_image(driver, wait_time=1.5):
    ...
    for y_coord in range(c_height, s_height, c_height):
        ...
        if s_height - y_coord < c_height:
            h = single_page.size[1] * \
                (c_height - (s_height - y_coord)) // c_height
            single_page = single_page.crop(
                (0, h, single_page.size[0], single_page.size[1]))
